...
def generate_imbalanced_data(n_samples=50, imbalance_ratio=0.9):
    X, y = make_classification(
        n_samples=n_samples,
        n_features=2,             # 2 features pour la visualisation
        n_redundant=0,            # pas de features redondantes
        n_informative=2,          # Toutes les features portent de l’information
        n_clusters_per_class=1,   # 1 sous-groupes par classe
        weights=[imbalance_ratio, 1 - imbalance_ratio],
        class_sep=2,              # séparation modérée
        flip_y=0.05,              # 5% des labels aléatoirement inversés (bruit)
        random_state=42,
    )
    # Étaler les points (multiplication par un facteur)
    X = X * 5.0  

    # Remplacer {0,1} par {-1,1}
    y = np.where(y == 0, -1, 1)
    return X, y
...
def main(argv):
    ...
    - Génère automatiquement un dataset déséquilibré
    - Charge les exemples dans le modèle
    ...
    # Génération auto du dataset déséquilibré
    X, y = generate_imbalanced_data(n_samples=300, imbalance_ratio=0.9)
    for xi, yi in zip(X, y):
        model.data.append((xi[0], xi[1], yi))
